...
const SignedFriendFacebookConnect = {
    ...
    view(vnode) {
        if (vnode.attrs.friendListVM.isLoading()) {
            return h.loader();
        }
        const total = vnode.attrs.friendListVM.total();
                ...
                m('.card.card-big', [
                    m('.w-row', [
                        ...
                        m('.w-col.w-col-4.u-text-center', [
                            ...
                            m('.u-marginbottom-20', [
                                _.map(vnode.state.mapWithAvatar(), item => m(`img.thumb.small.u-round.u-marginbottom-10[src="${item.avatar}"]`)),
                            ]),
