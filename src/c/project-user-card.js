            ...
            m('.w-row', [
                ...
                m('.w-col.w-col-8', [
                    m('.fontsize-small.link-hidden.fontweight-semibold.u-marginbottom-10.lineheight-tight[itemprop="name"]', [
                        m(`a.link-hidden${args.isDark ? '.link-hidden-white' : ''}[href="${_.isNull(userDetail.deactivated_at) ? `/users/${userDetail.id}` : 'javascript:void(0);' }"]`, { config: m.route,
                            onclick: () => {
                    ...
                    (!_.isNull(userDetail.deactivated_at) ? '' : m('ul.w-hidden-tiny.w-hidden-small.w-list-unstyled.fontsize-smaller.fontweight-semibold.u-margintop-20.u-marginbottom-20', [
                            (!_.isEmpty(userDetail.facebook_link) ? m('li', [
                                m(`a.link-hidden${args.isDark ? '.link-hidden-white' : ''}[itemprop="url"][href="${userDetail.facebook_link}"][target="_blank"]`, { onclick: h.analytics.event({ cat: 'project_view', act: 'project_creator_fb', lbl: userDetail.facebook_link, project: project() }) }, 'Perfil no Facebook')
                            ]) : ''), (!_.isEmpty(userDetail.twitter_username) ? m('li', [
                                m(`a.link-hidden${args.isDark ? '.link-hidden-white' : ''}[itemprop="url"][href="https://twitter.com/${userDetail.twitter_username}"][target="_blank"]`, { onclick: h.analytics.event({ cat: 'project_view', act: 'project_creator_twitter', lbl: userDetail.twitter_username, project: project() }) }, 'Perfil no Twitter')
                            ]) : ''),
                        _.map(userDetail.links, (link) => {
                            const parsedLink = h.parseUrl(link.link);

                            return (!_.isEmpty(parsedLink.hostname) ? m('li', [
                                m(`a.link-hidden${args.isDark ? '.link-hidden-white' : ''}[itemprop="url"][href="${link.link}"][target="_blank"]`, { onclick: h.analytics.event({ cat: 'project_view', act: 'project_creator_otherlinks', lbl: link.link, project: project() }) }, parsedLink.hostname)
                            ]) : '');
                        })
                    ])),
                    (!_.isEmpty(userDetail) ? [
                        ...
                        m(`a.w-button.btn.btn-terciary${args.isDark ? '.btn-terciary-negative' : ''}.btn-small[href=\'javascript:void(0);\']`, { onclick: h.analytics.event({ cat: 'project_view', act: 'project_creator_sendmsg', lbl: userDetail.id, project: project() }, ctrl.sendMessage) }, 'Contato')
                    ] : ''),
