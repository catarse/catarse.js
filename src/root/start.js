...
const start = {
    oninit() {
        ...
        const stats = console.warn("m.prop has been removed from mithril 1.0") || m.prop([]),
            categories = console.warn("m.prop has been removed from mithril 1.0") || m.prop([]),
            selectedPane = console.warn("m.prop has been removed from mithril 1.0") || m.prop(0),
            selectedCategory = console.warn("m.prop has been removed from mithril 1.0") || m.prop([]),
            featuredProjects = console.warn("m.prop has been removed from mithril 1.0") || m.prop([]),
            selectedCategoryIdx = console.warn("m.prop has been removed from mithril 1.0") || m.prop(-1),
            ...
            projectCategory = console.warn("m.prop has been removed from mithril 1.0") || m.prop('-1'),
            projectName = console.warn("m.prop has been removed from mithril 1.0") || m.prop(''),
            projectNameError = console.warn("m.prop has been removed from mithril 1.0") || m.prop(false),
            projectCategoryError = console.warn("m.prop has been removed from mithril 1.0") || m.prop(false),
            validateProjectForm = () => {
    ...
    view(vnode) {
        const stats = _.first(vnode.state.stats());
        const testimonials = () => _.map(vnode.state.testimonials, (testimonial) => {
        ...
        return m('#start', { config: h.setPageTitle(I18n.t('header_html', I18nScope())) }, [
            ...
            m('.w-section.section-large', [
                ...
                m('.w-container', [
                    m('.w-tabs.w-hidden-small.w-hidden-tiny', [
                        m('.w-tab-menu.w-col.w-col-4', _.map(vnode.state.paneImages, (pane, idx) => m(`btn.w-tab-link.w-inline-block.tab-list-item${(idx === vnode.state.selectedPane()) ? '.selected' : ''}`, {
                            onclick: h.analytics.event({ cat: 'project_start', act: 'start_solution_click', lbl: pane.label }, vnode.state.selectPane(idx))
                        }, pane.label))),
                        m('.w-tab-content.w-col.w-col-8', _.map(vnode.state.paneImages, (pane, idx) => m('.w-tab-pane', [
                            m(`img[src="${pane.src}"].pane-image${(idx === vnode.state.selectedPane()) ? '.selected' : ''}`)
                        ])))
                    ])
                ])
            ]),
            ...
            m('.w-hidden-small.w-hidden-tiny.section-categories', [
                m('.w-container', [
                    ...
                    m('.w-tabs', [
                        m('.w-tab-menu.u-text-center', _.map(vnode.state.categories(), category => m(`a.w-tab-link.w-inline-block.btn-category.small.btn-inline${(vnode.state.selectedCategoryIdx() === category.id) ? '.w--current' : ''}`, {
                            onclick: h.analytics.event({ cat: 'project_start', act: 'start_category_click', lbl: category.name }, vnode.state.selectCategory(category))
                        }, [
                            m('div', category.name)
                        ]))),
                        m('.w-tab-content.u-margintop-40', [
                            m('.w-tab-pane.w--tab-active', [
                                m('.w-row', (vnode.state.selectedCategoryIdx() !== -1) ? _.map(vnode.state.selectedCategory(), category => [
                                    m('.w-col.w-col-5', [
                                    ...
                                    m('.w-col.w-col-7', [
                                        ...
                                        !_.isEmpty(vnode.state.featuredProjects()) ? _.map(vnode.state.featuredProjects(), project => !_.isUndefined(project) ? m('.w-row.u-marginbottom-10', [
                                            m('.w-col.w-col-1', [
            ...
            m('.w-container', [
                ...
                m('.w-row.u-marginbottom-60', [
                    m('.w-col.w-col-6', _.map(vnode.state.questions.col_1, question => m(landingQA, {
                        question: question.question,
                        answer: question.answer,
                        onclick: h.analytics.event({ cat: 'project_start', act: 'start_qa_click', lbl: question.question })
                    }))),
                    m('.w-col.w-col-6', _.map(vnode.state.questions.col_2, question => m(landingQA, {
                        question: question.question,
                        answer: question.answer,
                        onclick: h.analytics.event({ cat: 'project_start', act: 'start_qa_click', lbl: question.question })
                    })))
                ...
                m('.w-container', [
                    ...
                    m('form[action="/projects/fallback_create"][method="GET"].w-row.w-form', {
                        onsubmit: (e) => {
                            ...
                            return vnode.state.validateProjectForm();
                        }
                    },
                        [
                            m('.w-col.w-col-2'),
                            m('.w-col.w-col-8', [
                                ...
                                m('input.w-input.text-field.medium.u-marginbottom-30[type="text"]', {
                                    name: 'project[name]',
                                    class: vnode.state.projectNameError() ? 'error' : '',
                                    onfocus: () => vnode.state.projectNameError(false),
                                    onchange: (e) => {
                                        h.analytics.oneTimeEvent({ cat: 'project_create', act: 'create_form_change', lbl: 'name' })(e);
                                        m.withAttr('value', vnode.state.projectName)(e);
                                    }
                                }),
                                ...
                                m('select.w-select.text-field.medium.u-marginbottom-40', {
                                    ...
                                    class: vnode.state.projectCategoryError() ? 'error' : '',
                                    onfocus: () => vnode.state.projectCategoryError(false),
                                    onchange: (e) => {
                                        h.analytics.oneTimeEvent({ cat: 'project_create', act: 'create_form_change', lbl: 'category' })(e);
                                        m.withAttr('value', vnode.state.projectCategory)(e);
                                    }
                                }, [
                                    m('option[value="-1"]', I18n.t('form.select_default', I18nScope())),
                                    _.map(vnode.state.categories(), category => m(`option[value="${category.id}"]`, { selected: vnode.state.projectCategory() === category.id }, category.name))
                                ])
                            ]),
                            m('.w-col.w-col-2'),
                            ...
                            m('.w-row.u-marginbottom-80', (vnode.state.projectNameError() || vnode.state.projectCategoryError()) ? m(
                                inlineError,
