...
const menu = {
    ...
    view(vnode) {
        return m('header.main-header', {
            class: vnode.state.menuCss()
        }, [
            m('.w-row', [
                m('.w-clearfix.w-col.w-col-8.w-col-small-8.w-col-tiny-8',
                    [
                        m('a.header-logo.w-inline-block[href=\'/?ref=ctrse_header\'][title=\'Catarse\']',
                            vnode.state.homeAttrs(),
                            m('img[alt=\'Logo big\'][src=\'/assets/catarse_bootstrap/logo_big.png\']')
                        ),
                        vnode.attrs.menuShort ? '' : m('div#menu-components', [
                            m('a.w-hidden-small.w-hidden-tiny.header-link.w-nav-link[href=\'/start?ref=ctrse_header\']', { oncreate: m.route.link }, 'Comece seu projeto'),
                ...
                m('.text-align-right.w-col.w-col-4.w-col-small-4.w-col-tiny-4', [
                    vnode.state.user ? m(menuProfile, { user: vnode.state.user }) : m('a.w-nav-link.header-link.w-nav-link.btn-edit.u-right[href=\'/pt/login?ref=ctrse_header\']', 'Login'),
                ])

            ]),
            vnode.attrs.menuShort ? '' : m('.header-controls-mobile.w-hidden-main.w-hidden-medium',
                [
